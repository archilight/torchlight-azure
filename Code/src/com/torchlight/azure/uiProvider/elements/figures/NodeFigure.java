...
public class NodeFigure extends com.archimatetool.editor.diagram.figures.elements.NodeFigure {
    ...
    protected void drawFigure(Graphics graphics) {
        ...
        if (azureName != null) {
        	...
        	if (!setType) {
        		getDiagramModelObject().setType(1);  // force the icon instead of block view
        		getDiagramModelObject().setTextPosition(ITextPosition.TEXT_POSITION_BOTTOM);  // force text to be bottom
        		setType = true;
        	}
        }
    ...
    protected void drawIcon(Graphics graphics) {
    	...
    	// format as large icon
    	key = "AzureIconLarge";	
    	
    	String iconURL = TorchlightAzurePlugin.mustReplaceIcon(getDiagramModelObject(), key);
    	if ( iconURL != null) {
			// draw Azure icon name if key is set
    		int h = this.bounds.height;
        	int w = this.bounds.width;
        	if (h > 100 && w > 150 ) {
        		// greater than 25% use large icon
        		point = new Point(this.bounds.x + this.bounds.width - 36, this.bounds.y + 3); // left top
        	}
        	else
        		point = new Point(this.bounds.x + (this.bounds.width/2) -16, this.bounds.y + 3); // middle top
